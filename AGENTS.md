...
This file provides guidance to AI coding agents (Claude Code, Cursor, Copilot, OpenCode, Gemini CLI, Antigravity, etc.) when working with the **pointfight-reactor** repository.

## Repository overview

pointfight-reactor was scaffolded from `phasekit`. It uses a phase-gated, audit-first workflow: work progresses through defined phases with explicit approval artifacts and git commits between each. Adapt this section to describe what pointfight-reactor actually does once the spec is filled in.
...
- `docs/SPEC.md` — what pointfight-reactor is supposed to do
